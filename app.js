...
const I18N = {
  en: {
    ...
    despatchReference: "Despatch reference",
    ...
    deliveryAdditionalStreet: "Delivery additional street",
    deliveryCity: "Delivery city",
    deliveryPostalCode: "Delivery postal code",
    deliveryCountry: "Delivery country",
    deliveryAddressLine: "Delivery note",
    sellerItemId: "Seller item ID",
    standardItemId: "Standard item ID",
  ...
  fr: {
    ...
    invoiceTypeCode: "Code type facture",
    customizationId: "ID de personnalisation",
    profileId: "ID de profil",
    invoicePeriodStart: "Début période facture",
    invoicePeriodEnd: "Fin période facture",
    orderReference: "Référence commande",
    despatchReference: "Référence expédition",
    endpointId: "ID endpoint",
    partyId: "ID partie",
    registrationName: "Nom légal",
    street: "Rue",
    city: "Ville",
    postalCode: "Code postal",
    country: "Pays",
    delivery: "Livraison",
    actualDeliveryDate: "Date de livraison",
    deliveryParty: "Partie livrée",
    deliveryStreet: "Rue livraison",
    deliveryAdditionalStreet: "Complément livraison",
    deliveryCity: "Ville livraison",
    deliveryPostalCode: "Code postal livraison",
    deliveryCountry: "Pays livraison",
    deliveryAddressLine: "Note livraison",
    sellerItemId: "ID article vendeur",
    standardItemId: "ID article standard",
    lineDiscount: "Remise",
    payment: "Paiement",
    paymentMeansCode: "Code moyen de paiement",
    iban: "IBAN",
    bic: "BIC",
    ...
    taxPercent: "Taux taxe",
    taxExemptionReason: "Motif exonération",
    lineExtension: "Montant lignes",
  },
  de: {
    language: "Sprache",
    ...
    invoiceTypeCode: "Rechnungstypcode",
    customizationId: "Customization-ID",
    profileId: "Profil-ID",
    invoicePeriodStart: "Abrechnungsbeginn",
    invoicePeriodEnd: "Abrechnungsende",
    orderReference: "Bestellreferenz",
    despatchReference: "Lieferscheinreferenz",
    endpointId: "Endpoint-ID",
    partyId: "Parteien-ID",
    registrationName: "Rechtlicher Name",
    street: "Straße",
    city: "Stadt",
    ...
    deliveryParty: "Lieferpartei",
    deliveryStreet: "Lieferstraße",
    deliveryAdditionalStreet: "Zusatzadresse Lieferung",
    deliveryCity: "Lieferstadt",
    deliveryPostalCode: "Liefer-PLZ",
    deliveryCountry: "Lieferland",
    deliveryAddressLine: "Lieferhinweis",
    sellerItemId: "Verkäufer-Artikelnummer",
    standardItemId: "Standard-Artikelnummer",
    lineDiscount: "Rabatt",
    payment: "Zahlung",
    paymentMeansCode: "Zahlungsartcode",
    iban: "IBAN",
    bic: "BIC",
    ...
    taxExemptionReason: "Steuerbefreiungsgrund",
    lineExtension: "Positionssumme",
  },
};
...
function renderInvoice(xmlDoc) {
  ...
  if (lines.length === 0) {
    rows.innerHTML = `<tr><td colspan="8">${t("noLines")}</td></tr>`;
  } else {
    for (const line of lines) {
      const tr = document.createElement("tr");
      tr.innerHTML = `
        <td>${valueByPath(line, "cbc:ID")}</td>
        <td>${valueByPath(line, "cac:Item/cbc:Name") !== "—" ? valueByPath(line, "cac:Item/cbc:Name") : valueByPath(line, "cac:Item/cbc:Description")}</td>
        <td>${valueByPath(line, "cac:Item/cac:SellersItemIdentification/cbc:ID")}</td>
        <td>${valueByPath(line, "cac:Item/cac:StandardItemIdentification/cbc:ID")}</td>
        <td class="right">${valueByPath(line, "cbc:InvoicedQuantity")}</td>
        <td class="right">${valueByPath(line, "cac:Price/cbc:PriceAmount")}</td>
        <td class="right">${valueByPath(line, "cac:AllowanceCharge/cbc:Amount")}</td>
